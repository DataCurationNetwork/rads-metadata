...
#read in pulled data
load("Combined_datacite_publisher_metadata.Rdata")

#read in Duke data
duke_cr <- read.csv(file="Duke_CrossrefDOIs_20220722.csv")
...
duke_cr1 <- duke_cr %>% 
  mutate(institution = "Duke", 
         ...
         publicationYear = gsub("[[:digit:]]{2}-[[:digit:]]{2}$", "", published.print)) %>% 
...
#Remove Duke datacite info, then merge the datasets
...
write.csv(all_dois_include3, file="All_IR_dois_20220727.csv", row.names = F)
