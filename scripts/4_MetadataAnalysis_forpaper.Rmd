...
(by_publisher_plot_collapse2 <-  by_publisher_collapse %>% 
   ...
   facet_wrap(~institution)+
    labs(x = "Publisher", y="Count of data and sofware DOIs", caption = "*Michigan ENCODE bar truncated for visualization") +
    theme_bw() +
    theme(legend.position = "bottom", legend.title.align = .5))

ggsave(by_publisher_plot_collapse2, filename = "figures/fig3_DOI_by_Institution.png", device = "png",  width = 8, height = 6, units="in")

ggsave(by_publisher_plot_collapse2, filename = "figures/fig3_DOI_by_Institution.tif", device = "tiff",  width = 8, height = 6, units="in")
...
pubshortnames <- data.frame(publisher = c(top12pubs, "Other"), 
                            shortname = c("EN", "FOL", "Z", "DR", "IR", "FG", "ICP", "HD", "TF", "NPD","VTI","MIV","O"))
...
plotforlegend <- by_publisher_collapse %>% 
  group_by(institution) %>% 
  mutate(Percent = count/sum(count)*100) %>% 
  filter(publisher %in% top12pubs) %>% 
  ggplot(aes(x=institution, y=Percent)) +
...
publegend <- get_legend(plotforlegend)


##PLOT A
plotAdata <-  by_publisher_collapse %>% 
  mutate(publisher = ifelse(publisher %in% top10pubs, publisher, "Other"), 
         institution = ifelse(institution == "Washington U", "Wash U", institution)) %>%
...
ggsave(plot = combined_pub_plots, filename="figures/Fig4_DOIDistribution.png", device = "png", width = 10.5, height = 8, units = "in")
...
by_year_nrepos %>% 
  ...
  annotate(geom="text", x= 11.2, y=(by_year_nrepos$npublishers[which(by_year_nrepos$institution== "Washington U" & by_year_nrepos$publicationYear == 2022)]-.5), label="Wash U", hjust=0) +
...
ggsave(filename="figures/Fig2_N_Repo_by_Year.png", device = "png")
ggsave(filename="figures/Fig2_N_Repo_by_Year.tiff", device = "tiff")
...
* Affiliation: Resource Author Affiliation (creators)
...
for_metadata <- all_dois_collapsed %>% 
  select(institution, publisher, group, DOI, creators, publicationYear, relatedIdentifiers, fundingReferences) %>% 
  mutate(RowID = 1:nrow(.))
