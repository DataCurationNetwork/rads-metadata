...
(by_publisher_data_plot_collapse <-  by_publisher_data_dc_collapse %>% 
    ...
    #scale_y_continuous(breaks = seq(from = 0, to=5000, by=500)) +
    #coord_cartesian(ylim = c(0,5000)) +
    labs(x = "Institution", y="Count of Data DOIs") +
    theme_bw() +
    theme(legend.position = "bottom", legend.title.align = .5))
...
Collapse by version. Figshare updates the DOI with a .vX for each version update. 
```{r}
all_dois_collapsed %>% 
...
Remove multiple versions of the same dataset. 
```{r}
all_dois_collapsed_versions <- all_dois_collapsed %>% 
  mutate(DOIvstripped = gsub("\\.v[[:digit:]]+", "", DOI)) %>% 
  filter(!duplicated(DOIvstripped))
```

Repeat graphs when multiple versions of the same dataset are removed. 


## Overall Count of Data DOIs

```{r}
data_dois_collapse_versions <- all_dois_collapsed_versions %>% 
  filter(resourceTypeGeneral == "Dataset")

by_publisher_data_collapse_versions <- data_dois_collapse_versions %>% 
  group_by(publisher, institution) %>% 
  summarize(count=n()) %>% 
  arrange(institution, desc(count))
```
...
Table of publisher counts
```{r}
by_publisher_data_collapse_versions_table <- by_publisher_data_collapse_versions %>% 
  pivot_wider(names_from = institution, 
              values_from = count, 
              values_fill = 0) %>% 
  rowwise %>% 
...
Write out the table of data publishers
```{r}
write.csv(by_publisher_data_collapse_versions_table, file="data_summary_data/Counts of Data Publishers By Insitituion - Collapsed by container and version.csv", row.names = F)
```
...
```{r}
by_publisher_data_dc_collapse_v <- data_dois_collapse_versions %>% 
  filter(group != "Affiliation - CrossRef") %>% 
  group_by(publisher, institution) %>% 
  summarize(count=n()) %>% 
  arrange(institution, desc(count))
...
#table of  publishers - data
by_publisher_data_dc_collapse_v_table <- by_publisher_data_dc_collapse_v %>% 
  pivot_wider(names_from = institution, 
              values_from = count, 
              values_fill = 0) %>% 
  rowwise %>% 
...
Look at publishers based on rank of number of DOIs
```{r}
by_publisher_data_dc_collapse_v_table %>% 
  group_by(publisher) %>% 
  summarize(count=sum(Total)) %>% 
  arrange(desc(count)) %>% 
  mutate(pubrank = order(count, decreasing = T)) %>% 
...
Look at the top 7 publishers - how many does this capture?
```{r}
top7pubs <- by_publisher_data_dc_collapse_v_table$publisher[1:7]
