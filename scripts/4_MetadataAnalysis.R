...
## Updated 2022-11-15
#################################################
## NOTE: this script takes in data that is generated
## by 1_MetadataPull_datacite.R
################################################
#clear workspace
rm(list=ls())
...
#remove duplicate IDs (dois)
dups <- all_dois$id[which(duplicated(all_dois$id))]

# all_dois %>% 
#   filter(id %in% dups) %>% 
#   View() 
#ahh most of the duplicates are multi-institutional papers. Will leave them in
...
all_dois$publisher[grep("Consortium for Political", all_dois$publisher)] <- "ICPSR - Interuniversity Consortium for Political and Social Research"

## 1. Remove morphosource and replace with affiliation

morphodat <- all_dois %>% 
  filter(publisher_plus == "Duke-MorphoSource Media")
#Looks like affiliation is not included 

#for now, just remove morphosource data
all_dois2 <- all_dois[-which(all_dois$publisher_plus == "Duke-MorphoSource Media"),]

#make sure dataset is capitalized in both
...
## With both DataCite & CrossRef data Included ####
top8pubs <- by_publisher_data_table$publisher[1:8]
...
## What about cross ref affiliation data?
(by_group_publisher_data_plot <- all_dois2 %>% 
    group_by(publisher, institution, group) %>% 
    summarize(count=n()) %>% 
    filter(count > 150) %>% 
    ggplot(aes(x=institution, y=count, fill=publisher)) +
    geom_bar(stat="identity", position=position_dodge(preserve = "single")) +
    ...
    scale_y_continuous(breaks = seq(from = 0, to=5000, by=500)) +
    coord_cartesian(ylim = c(0,5000)) +
    labs(x = "Institution", y="Count of Data DOIs", caption = "Note: Michigan ENCODE and Dataverse bars cutoff for scaling") +
    theme_bw() +
    theme(legend.position = "bottom", legend.title.align = .5))

ggsave(by_group_publisher_data_plot, filename = "figures/Counts of Data DOIs by Institution and Source.png", device = "png",  width = 15, height = 8, units="in")
...
### Recreating the Main Graph from Ted - Data Cite affiliation & IR data only ####
...
top6pubs <- by_publisher_data_dc_table$publisher[1:6]
...
all_dois_forcsv <- all_dois[,which(unlist(lapply(lapply(all_dois2, class), length)) == 1)]
...
## Collapsing by container to capture DOIs from the same study 
by_container <- 
all_dois2 %>% 
...
#examining a few
#zenodo:  container_identifier == "10.5852/ejt.2012.28"
#https://zenodo.org/record/3858480#.Y3U6N-zMK7Y
#seems to be pieces all associated with an article

#zenodo:	10.1206/0003-0090.439.1.1 
#data and figures from a jounral article: https://zenodo.org/record/4633195#.Y3U65ezMK7Y
#https://bioone.org/journals/bulletin-of-the-american-museum-of-natural-history/volume-439/issue-1/0003-0090.439.1.1/A-Revision-of-the-Didelphid-Marsupial-Genus-MarmosaPart-2-Species/10.1206/0003-0090.439.1.1.full?tab=ArticleLinkFigureTable

#dataverse 10.7910/dvn/txskka/w4q5ud
#yes, the 58 files from that record
filter(all_dois2, container_identifier == "doi:10.7910/DVN/TXSKKA") %>% View
...
#check
#all_dois2[containerdups,] %>% View

all_dois_collapsed <- all_dois2[-containerdups,]

## New tables for collapsed #####
...
#redo the plots with the collapsed data
